...
class PsSimulatorUiTest(unittest.TestCase):
    def test_rendered_ui_fetches_repo_local_sample_dataset_copy(self) -> None:
        ...
        self.assertIn("const opponentSubmissionCpuLimits", html)
        self.assertIn("const opponentSubmissionCpuScoreWeights", html)
        self.assertIn("function buildOpponentSubmissionCpu()", html)
        self.assertIn("function opponentSubmissionCpuCandidateSet(selfDeckIds)", html)
        self.assertIn("function generateOpponentSubmissionCpuSplits(deckIds)", html)
        self.assertIn("function evaluateOpponentSubmissionCpuPlan(split, deckSetIndex, splitIndex)", html)
        self.assertIn("function opponentSubmissionCpuHtml(cpu)", html)
        self.assertIn('id="opponent-submission-cpu"', html)
        self.assertIn("相手側想定提出案CPU", html)
        self.assertIn("バランス案", html)
        self.assertIn("安定案", html)
        self.assertIn("攻撃案", html)
        self.assertIn("MVPではBattle 1-3中心の評価です。Battle 4/5の使用済みデッキ制約は後続課題です。", html)
        self.assertIn("逆向きmatchupの自動補完は行いません。", html)
        self.assertIn("deckCandidates / provisionalDecks / matchup_matrix / UI選択可能デッキ", html)
        self.assertIn("3/2/2分割", html)
        self.assertIn("強有利対面", html)
        self.assertIn("危険対面", html)
        self.assertIn("opponentSubmissionCpuHtml(buildOpponentSubmissionCpu())", html)
