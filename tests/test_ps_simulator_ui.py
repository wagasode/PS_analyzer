...
class PsSimulatorUiTest(unittest.TestCase):
    def test_rendered_ui_fetches_repo_local_sample_dataset_copy(self) -> None:
        ...
        self.assertIn("active-throw-advice-summary", html)
        self.assertIn("throwAdviceDetailsOpen", html)
        self.assertIn("const matchupStatsThresholds", html)
        self.assertIn("const throwAdviceScoreWeights", html)
        ...
        self.assertIn("function activeThrowAdviceSummaryText(advice)", html)
        self.assertIn("function activeThrowAdviceDetailsHtml(advice)", html)
        self.assertIn("buildBattleThrowAdvice(battleThrowDeckSetForRound(active.roundNumber))", html)
        ...
        self.assertIn("function buildOpponentCpuPick(selfDeckId, opponentDeckIds)", html)
        self.assertIn("総合おすすめ", html)
        self.assertIn("安定投げ", html)
        self.assertIn("攻撃的投げ", html)
        self.assertIn("危険候補", html)
        self.assertIn("相手CPU", html)
        self.assertIn("一部の相性データが欠損しているため、評価は参考値です。", html)
