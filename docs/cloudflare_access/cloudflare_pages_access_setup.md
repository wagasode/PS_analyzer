...
| 初期 production URL | `https://ps-analyzer.pages.dev`。当面は deploy 検証用URLとして扱う |
| 独自ドメイン | チーム限定本番URLには必要になる可能性が高い。#50時点では未設定 |
| Access 認証方式 | Google login |
| 許可方式 | チームメンバーの Google アカウントを個別 allowlist する |
| One-time PIN | 初期必須ではない。Google login で詰まる場合の補助候補として残す |
| preview URL | `Restrict previews` で Access 保護対象に含める |
| GitHub Pages 停止/無害化 | 限定公開本番URLが確保されてから #47 で扱う |
...
| custom domain | チーム限定本番URL用に後続で検討する |
...
## 2026-05-20 追加確認: production `pages.dev` の扱い

Cloudflare Pages project `ps-analyzer` の Settings > General で確認した結果、`Restrict previews` は preview deployment URL だけを Access 保護する機能でした。Cloudflare の画面上でも、production `pages.dev` と custom domain は Zero Trust 側で別管理と案内されています。

一方、Zero Trust の self-hosted application で production `ps-analyzer.pages.dev` を保護しようとすると、Cloudflare に onboard 済みの domain が必要になります。そのため、`https://ps-analyzer.pages.dev/` は当面 dashboard deploy 検証用URLとして扱い、チーム限定公開の本番URLとは扱いません。

チーム限定公開の本番URLは、custom domain を Cloudflare に onboard し、その hostname を Cloudflare Access application で保護する方式を第一候補にします。#47「GitHub Pages直URLを停止・無害化する」は、Cloudflare Pages への deploy 成功だけで開始せず、限定公開本番URLが確保されてから進めます。
...
Access application は、preview URL と custom domain の本番URLを分けて扱います。production `ps-analyzer.pages.dev` は当面 deploy 検証用URLであり、チーム限定公開の本番URLとは扱いません。

### production custom domain

1. Cloudflare に custom domain を onboard する。
2. Cloudflare Pages project `ps-analyzer` に custom domain を割り当てる。
3. Cloudflare Zero Trust dashboard を開く。
4. Access applications で self-hosted application を作成する。
5. application 名は `PS Analyzer production` など、production 用だと分かる名前にする。
6. public hostname に custom domain の hostname を設定する。
7. login method に Google identity provider を選ぶ。
8. allow policy を作成し、チームメンバーの Google account email を個別に include する。
9. policy で `Everyone`、全ドメイン許可、広い email domain 許可を使っていないことを確認する。
10. Access application の session duration、app launcher 表示有無、ログ記録方針を checklist に記録する。

### preview URL

preview deployment は production と同じくチーム限定公開として扱います。

1. Pages project の `Restrict previews` / preview deployment protection 設定を確認する。
2. preview deployment 用の Access policy を有効にする。
3. preview wildcard が `*.ps-analyzer.pages.dev` 相当を保護できているか確認する。
4. production custom domain と preview wildcard の保護範囲を別々に記録する。
5. branch alias URL と deployment hash URL のどちらが発行されるかを確認し、両方を確認対象に含める。

`Restrict previews` は preview deployment URL だけを保護します。production `pages.dev` と custom domain は Zero Trust 側で別管理です。
...
| production custom domain | 未ログイン / private window | Cloudflare Access login に誘導される |
| production custom domain | allowlist 外の Google account | Access denied になる |
| production custom domain | allowlist 内の Google account | Pages の内容を表示できる |
...
- Pages project名: `ps-analyzer`
- production URL: `https://ps-analyzer.pages.dev`。当面は deploy 検証用URLとして扱う
...
- dashboard の origin は deploy 検証用の `https://ps-analyzer.pages.dev`、preview URL、後続で確保する production custom domain を分けて再整理する。
...
- Cloudflare Pages への deploy 成功だけでは進めない。production custom domain + Cloudflare Access の限定公開本番URLが確認できてから進める。
...
- deploy 検証用 `pages.dev`、production custom domain、preview URL の確認手順。
